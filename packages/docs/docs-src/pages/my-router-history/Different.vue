<template>
    <Doc>
        ...
        <H2>解决的问题</H2>
        <P>在web路由中，经常会将app路由的状态保存到浏览器的URL中，把用户的浏览器记录保存到浏览器的浏览记录中。这是因为用户可以刷新页面，一旦刷新页需要浏览器的URL记录当前用户的位置，使得刷新后能够回到当前页面。同时，浏览器的地址栏上面有前进、后退按钮，用户希望这些导航按钮能操作他们浏览记录，因此需要用浏览器的历史记录保存用户真正的访问记录。</P>
        <P>然而将App的访问记录保存在浏览器的访问记录中，会产生一些问题。如：无法修改和删除已经浏览记录、无法查询用户的浏览器记录、无法监听返回事件、用户可以通过<Strong>地址栏的前进按钮</Strong>访问已经离开的页面。这些都给App开发造成了麻烦。</P>
        
        <H3>无法监听返回事件</H3>
        <P>安卓浏览器提供了系统的返回按钮，这刚好和浏览器的<Srong>退回按钮</Srong>相对应，不过与安卓的返回按钮不同的事，我们无法监听浏览器的<Srong>退回按钮</Srong>的点击以及阻止默认的事件响应。例如我们在页面中打开一个对话框，单用户点击返回按钮时，其实是希望关闭这个对话框（如极大多数安卓APP那样），而实际表现却是返回上一个页面，这样不符合用户的操作习惯。</P>
        ...
        <H2>原理</H2>
        <P>与传统web的SPA路由不同，<Strong>my-router-history</Strong>不仅仅基于hash和history的state相关API，其最核心的技术是<Strong>返回键劫持</Strong>。</P>
    </Doc>
</template>
