<doc>
    ...
    <p><strong>my-reoute-history</strong>是<a href="https://github.com/laden666666/my-router">my-reoute</a>的一个子项目，旨在用于浏览器上实现高仿APP式的单页面路由。<strong>my-reoute-history</strong>屏蔽了浏览器的诸多行为：记录用户历史记录、前进按钮等，并监听用户返回动作，使得WebApp路由变得更像App。可用于PWA路由开发、WebApp路由开发、微信小程序路由移植等场景。</p>

    <h2>主要功能</h2>

    <p><strong>my-reoute-history</strong>的主要功能如下：</p>
    <h3>增加返回页面</h3>
    <p><strong>my-reoute-history</strong>会在用户进入Web页面后，插入一个专门监听返回行为的页面。如下图用户进入网页后后，会自动地增加一条名为“返回”的浏览记录。</p>
    ...
    <h3>禁用地址栏的前进按钮</h3>
    <p>浏览器中，用户返回上一页后，地址栏的“前进按钮”就会变为可点击状态，用户点击“前进按钮”会回到之前的页面。而<strong>my-reoute-history</strong>屏蔽了这一行为，因为“前进按钮”在App中也是没有的。如下图，用户点击返回后，前进按钮仍然是不可用状态。</p>
