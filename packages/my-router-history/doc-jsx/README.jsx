<doc>
    ...
    <p><strong>my-reoute-history</strong>是<a href="../../">my-reoute</a>的一个子项目，旨在用于浏览器上实现高仿APP式的单页面路由。<strong>my-reoute-history</strong>屏蔽了浏览器的诸多行为：记录用户历史记录、前进按钮等，并监听用户返回动作，使得WebApp路由变得更像App。可用于PWA路由开发、WebApp路由开发、微信小程序路由移植等场景。</p>

    <h2>主要功能</h2>

    <p><strong>my-reoute-history</strong>的主要功能如下：</p>
    <h3>增加返回页面</h3>
    <p><strong>my-reoute-history</strong>会在用户进入Web页面后，插入一个专门监听返回行为的页面。如下图用户进入网页后后，会自动地增加一条名为“返回”的页面。</p>
    <img src="./GIF.gif" alt=""/>

    <h3>用户跳转页面后，不产生浏览记录</h3>
    <p>浏览器的地址栏会收集用户跳转页面的记录，<strong>my-reoute-history</strong>屏蔽了这一行为，这主要是阻止防止用户跳转到历史记录的某个页面，因为App是没有这种功能的，App会试图用自己的导航去引导用户，而历史记录会让用户跳出App精心设计的导航，使得工作流变动混乱，用户体验变得更差。</p>
    
    <p>如下图，用户在地址栏输入地址“/demo”后，页面跳转，但是浏览器中仅存在“返回”一条记录，看不到“首页”记录。</p>
    <img src="./GIF2.gif" alt=""/>

    <h3>返回上一页，地址栏的前进按钮不可用</h3>
    <p>浏览器中，用户返回上一页，地址栏的“前进按钮”就会变为可点击状态，用户点击前进按钮会回到之前的页面。而<strong>my-reoute-history</strong>屏蔽了这一行为，因为“前进按钮”在App中也是不多见的。如下图，用户点击返回后，前进按钮仍然是不可用状态。</p>
    ...
    <p>虽然<strong>my-reoute-history</strong>可以屏蔽用户修改URL，但是<strong>my-reoute-history却不建议开发者开启这项功能</strong>，因为我们毕竟是在浏览器中，禁止修改URL，会降低用户的使用体验，这也违反<strong>my-reoute-history</strong>设计的初衷。</p>
